...
async def first_choosing(message: types.Message, state: FSMContext):
    ...
    if (answer != 1 and answer != 2 and answer != 3):
        return await bot.send_message(message.from_user.id,
                                      "К сожалению, такого товара нет в наличии, попробуйте выбрать другой")
...
async def second_choosing(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id,
                           "Выберите хлебобулочное изделие\n1)Батон белого\n2)Пол чёрного\n3)Пирожёк с повидлом")
    await Test.Q3.set()
...
async def second_choosing(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id,
                           "Ваш заказ:\nМолочный продукт:\n{}\nМясо или рыба:\n{}\nХлебобулочное изделие:\n{}".format(
                               milk[data.get("q1")], meat_fish[data.get("q2")], bread[data.get("q3")]))
    await Test.Q0.set()
